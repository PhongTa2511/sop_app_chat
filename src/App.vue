...
const isGroupMuted = (groupID) => {
  try {
    ...
    return Boolean(
      map && typeof map === "object" && Number(map[String(gid)]) === 1,
    );
  } catch (_) {
    return false;
  }
      ...
      <VCard>
        ...
        <VCardText>
          <div v-if="notification.type === 2" class="text-center pb-4">
            <VIcon color="green" size="80"> mdi-check-circle-outline </VIcon>
          </div>
          <div v-if="notification.type === 0" class="text-center pb-4">
            <VIcon color="red" size="80"> mdi-close-circle-outline </VIcon>
          </div>
          <div v-if="notification.type === 1" class="text-center pb-4">
            <VIcon color="orange" size="80"> mdi-alert-circle-outline </VIcon>
          </div>
