<template>
  ...
  <v-dialog v-model="isShowAddNew" max-width="450px">
    <v-card>
      <v-card-title>
        <span class="text-h5">Thêm hồ sơ phát sinh công việc</span>
      </v-card-title>
      <v-card-text>
        <v-text-field
          label="Tên hồ sơ"
          v-model="newDocument.Conclusion"
          class="mb-2"
        ></v-text-field>
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { title: "Tên hồ sơ", key: "Conclusion", sortable: false },
    ...
    btCreateGSPDocument() {
      CreateGSPDocument({
        DocumentInfo: {
          ...
          Conclusion: this.newDocument.Conclusion,
        },
      }).then((res) => {
        if (res.RespCode == 0) {
      ...
      GetGSPDocumentLst({
        ...
        this.fileLst = res.Data.map((item, index) => {
          ...
          return {
            ...item,
            Key: index + 1 + a,
            DateReceptShow: formatDateDisplayDDMMYY(item.DateRecept),
